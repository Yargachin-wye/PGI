...
const Color& BMP_File::GetPixelColor(int x, int y)
{
	...
	if (usePalitra)
	{
		...
		return m_palitra[GetBits(m_pixels[y][byteInd], bitIndMod, m_info->bitperpixel)];
	}
...
/*void BMP_File::SetPalitra(std::vector<Color> palitra)
{
	m_palitra = std::move(palitra);
	m_palitra.resize(GetColorTableMaxSize());
}*/
...
unsigned long long BMP_File::GetDiffColor(Color x, Color y)
{
	...
	diff += 0.31 * (pow(abs(x.values[0] - y.values[0]), 2));
	diff += 0.59 * (pow(abs(x.values[1] - y.values[1]), 2));
	diff += 0.1 * (pow(abs(x.values[2] - y.values[2]), 2));
